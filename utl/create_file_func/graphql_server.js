...
function buildFieldItem(field) {
  return `${field.name}: { type: ${checkForRequired(
    ...
    "front"
  )}${tableTypeToGraphqlType(field.type)}${checkForMultipleValues(
    field.multipleValues,
    "back"
  )}${checkForRequired(field.required, "back")} }`;
}
...
function createSubQuery(field, tables, database) {
  ...
  const refFieldName =
    tables[field.relation.tableIndex].fields[field.relation.fieldIndex].name;
  const refFieldType =
    tables[field.relation.tableIndex].fields[field.relation.fieldIndex].type;
  ...
  if (
    field.relation.refType === "one to many" ||
    field.relation.refType === "many to many"
  ) {
...
function addMutation(table, database) {
  ...
  for (const fieldIndex in table.fields) {
    ...
    query += `${tab}${tab}${tab}${tab}${buildFieldItem(
